...
// ${APOLLO_DIR}/storage-pools/<pool>/containers
...
// ${APOLLO_DIR}/storage-pools/<pool>/snapshots
...
func (s *storageBtrfs) StoragePoolCreate() error {
		...
		if filepath.IsAbs(source) {
			...
			if isBlockDev {
				...
				if isBtrfsSubVolume(source) {
					...
					} else if strings.HasPrefix(cleanSource, apolloDir) {
						if cleanSource != poolMntPoint {
							return fmt.Errorf("BTRFS subvolumes requests in APOLLO directory \"%s\" are only valid under \"%s\"\n(e.g. source=%s)", shared.VarPath(), shared.VarPath("storage-pools"), poolMntPoint)
	...
	var err1 error
	var devUUID string
	mountFlags, mountOptions := apolloResolveMountoptions(s.getBtrfsMountOptions())
...
func (s *storageBtrfs) StoragePoolMount() (bool, error) {
	...
	if waitChannel, ok := apolloStorageOngoingOperationMap[poolMountLockID]; ok {
	...
	apolloStorageOngoingOperationMap[poolMountLockID] = make(chan bool)
	apolloStorageMapLock.Unlock()
	...
	removeLockFromMap := func() {
		...
		if waitChannel, ok := apolloStorageOngoingOperationMap[poolMountLockID]; ok {
			close(waitChannel)
			delete(apolloStorageOngoingOperationMap, poolMountLockID)
		}
		apolloStorageMapLock.Unlock()
	}
	...
	if filepath.IsAbs(source) {
		...
		if !isBlockDev && cleanSource == loopFilePath {
			// If source == "${APOLLO_DIR}"/disks/{pool_name} it is a
...
func (s *storageBtrfs) StoragePoolUmount() (bool, error) {
	...
	apolloStorageMapLock.Lock()
	if waitChannel, ok := apolloStorageOngoingOperationMap[poolUmountLockID]; ok {
		apolloStorageMapLock.Unlock()
		if _, ok := <-waitChannel; ok {
			logger.Warnf("Received value over semaphore. This should not have happened.")
		}
	...
	apolloStorageOngoingOperationMap[poolUmountLockID] = make(chan bool)
	apolloStorageMapLock.Unlock()
	...
	removeLockFromMap := func() {
		...
		if waitChannel, ok := apolloStorageOngoingOperationMap[poolUmountLockID]; ok {
			close(waitChannel)
			delete(apolloStorageOngoingOperationMap, poolUmountLockID)
		}
		apolloStorageMapLock.Unlock()
	}
...
func (s *storageBtrfs) ContainerCreate(container container) error {
	...
	// thus be
	// ${APOLLO_DIR}/storage-pools/<pool>/containers/. The btrfs tool will
...
func (s *storageBtrfs) ContainerCreateFromImage(container container, fingerprint string) error {
	...
	// ${APOLLO_DIR}/storage-pools/<pool>/containers/<name>
...
func (s *storageBtrfs) ContainerDelete(container container) error {
	...
	// ${APOLLO_DIR}/snapshots/<container_name> -> ${POOL}/snapshots/<container_name>
...
func (s *storageBtrfs) ContainerSnapshotCreate(snapshotContainer container, sourceContainer container) error {
	...
	// thus be
	// ${APOLLO_DIR}/storage-pools/<pool>/snapshots/. The btrfs tool will
	// complain if the intermediate path does not exist, so create it if it
	// doesn't already.
...
func (s *storageBtrfs) ImageCreate(fingerprint string) error {
	...
	// be
	// ${APOLLO_DIR}/storage-pools/<pool>/images/. The btrfs tool will
	// complain if the intermediate path does not exist, so create it if it
	// doesn't already.
	...
	// ${APOLLO_DIR}/storage-pools/<pool>/images/<fingerprint>@<pool>_tmp.
...
func (s *storageBtrfs) ImageDelete(fingerprint string) error {
	...
	// do this is ${APOLLO_DIR}/storage-pools/<pool>/images/<fingerprint>.
	imageSubvolumeName := getImageMountPoint(s.pool.Name, fingerprint)
...
func (s *btrfsMigrationSourceDriver) SendWhileRunning(conn *websocket.Conn, op *operation, bwlimit string, containerOnly bool) error {
	...
	// Deal with sending a snapshot to create a container on another APOLLO
	// instance.
