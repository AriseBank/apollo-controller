...
func deviceTaskBalance(d *Daemon) {
	...
	err = cGroupSet("cpuset", "/mercury", "cpuset.cpus", effectiveCpus)
	if err != nil && shared.PathExists("/sys/fs/cgroup/cpuset/mercury") {
		logger.Warn("Error setting apollo's cpuset.cpus", log.Ctx{"err": err})
	}
...
func deviceUSBEvent(d *Daemon, usb usbDevice) {
	...
	for _, name := range containers {
		...
		c, ok := containerIf.(*containerMERCURY)
		if !ok {
			logger.Errorf("got device event on non-MERCURY container?")
			return
		}
