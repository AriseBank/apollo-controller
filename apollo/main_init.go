...
import (
	...
	"github.com/AriseBank/apollo-controller/shared/cmd"
	"github.com/AriseBank/apollo-controller/shared/logger"
)

// CmdInitArgs holds command line arguments for the "apollo init" command.
...
// CmdInit implements the "apollo init" command line.
type CmdInit struct {
	Context         *cmd.Context
	Args            *CmdInitArgs
...
func (cmd *CmdInit) Run() error {
	...
	// Connect to APOLLO
	client, err := apollo.ConnectAPOLLOUnix(cmd.SocketPath, nil)
	if err != nil {
		return fmt.Errorf("Unable to talk to APOLLO: %s", err)
	}
	...
	cmd.Context.Output("APOLLO has been successfully configured.\n")
...
func (cmd *CmdInit) fillDataAuto(data *cmdInitData, client apollo.ContainerServer, backendsAvailable []string, existingPools []string) error {
...
func (cmd *CmdInit) fillDataInteractive(data *cmdInitData, client apollo.ContainerServer, backendsAvailable []string, existingPools []string) error {
	...
	if err != nil && bridge != nil {
		return fmt.Errorf("APOLLO managed bridges require \"dnsmasq\". Install it and try again.")
	}
...
func (cmd *CmdInit) fillDataPreseed(data *cmdInitData, client apollo.ContainerServer) error {
...
func (cmd *CmdInit) fillDataWithCurrentServerConfig(data *cmdInitData, client apollo.ContainerServer) error {
...
func (cmd *CmdInit) fillDataWithCurrentDefaultProfile(data *cmdInitData, client apollo.ContainerServer) {
...
func (cmd *CmdInit) fillDataWithStorage(data *cmdInitData, storage *cmdInitStorageParams, existingPools []string) error {
	...
	// When apollo init is rerun and there are already storage pools
		...
		if len(data.Profiles) != 0 {
			...
			for k, v := range defaultProfile.Devices {
				if v["path"] == "/" && v["source"] == "" {
					...
					// with a clean APOLLO but with a pool property key existing in
...
func (cmd *CmdInit) apply(client apollo.ContainerServer, data *cmdInitData) error {
...
// Try to revert the state to what it was before running the "apollo init" command.
...
func (cmd *CmdInit) initConfig(client apollo.ContainerServer, config map[string]interface{}) (reverter, error) {
...
func (cmd *CmdInit) initPool(client apollo.ContainerServer, pool api.StoragePoolsPost) (reverter, error) {
...
func (cmd *CmdInit) initPoolCreate(client apollo.ContainerServer, pool api.StoragePoolsPost) (reverter, error) {
	reverter := func() error {
...
func (cmd *CmdInit) initPoolUpdate(client apollo.ContainerServer, pool api.StoragePoolsPost, currentPool api.StoragePoolPut) (reverter, error) {
	reverter := func() error {
...
func (cmd *CmdInit) initNetwork(client apollo.ContainerServer, network api.NetworksPost) (reverter, error) {
...
func (cmd *CmdInit) initNetworkCreate(client apollo.ContainerServer, network api.NetworksPost) (reverter, error) {
	reverter := func() error {
...
// revert it to its original state.
func (cmd *CmdInit) initNetworkUpdate(client apollo.ContainerServer, network api.NetworksPost, currentNetwork api.NetworkPut) (reverter, error) {
	reverter := func() error {
...
func (cmd *CmdInit) initProfile(client apollo.ContainerServer, profile api.ProfilesPost) (reverter, error) {
	var reverter func() error
...
func (cmd *CmdInit) initProfileCreate(client apollo.ContainerServer, profile api.ProfilesPost) (reverter, error) {
	reverter := func() error {
...
// revert it to its original state.
func (cmd *CmdInit) initProfileUpdate(client apollo.ContainerServer, profile api.ProfilesPost, currentProfile api.ProfilePut) (reverter, error) {
	reverter := func() error {
...
func (cmd *CmdInit) validateArgsAuto(availableStoragePoolsDrivers []string) error {
	if !shared.StringInSlice(cmd.Args.StorageBackend, supportedStoragePoolDrivers) {
		return fmt.Errorf("The requested backend '%s' isn't supported by apollo init.", cmd.Args.StorageBackend)
	}
...
func (cmd *CmdInit) askStorage(client apollo.ContainerServer, existingPools []string, availableBackends []string) (*cmdInitStorageParams, error) {
	...
	for {
		...
		if !shared.StringInSlice(storage.Backend, supportedStoragePoolDrivers) {
			return nil, fmt.Errorf("The requested backend '%s' isn't supported by apollo init.", storage.Backend)
		}
		...
		if cmd.Context.AskBool(question, "yes") {
			if storage.Backend == "ceph" {
				...
				storage.Config["ceph.osd.pool_name"] = cmd.Context.AskString("Name of the OSD storage pool [default=apollo]: ", "apollo", nil)
			...
			if storage.Backend == "ceph" {
				...
				storage.Config["source"] = cmd.Context.AskString("Name of the existing OSD storage pool [default=apollo]: ", "apollo", nil)
...
your Linux distribution and run "apollo init" again afterwards.
...
APOLLO can re-use your container's own allocation to avoid the problem.
...
// Ask if the user wants to expose APOLLO over the network, and collect
// the relevant parameters if so.
func (cmd *CmdInit) askNetworking() *cmdInitNetworkingParams {
	if !cmd.Context.AskBool("Would you like APOLLO to be available over the network (yes/no) [default=no]? ", "no") {
		return nil
	}
	networking := &cmdInitNetworkingParams{}
	...
	networking.Address = cmd.Context.AskString("Address to bind APOLLO to (not including port) [default=all]: ", "all", isIPAddress)
	...
	networking.Port = cmd.Context.AskInt("Port to bind APOLLO to [default=8443]: ", 1, 65535, "8443")
...
// the relevant parameters if so.
func (cmd *CmdInit) askBridge(client apollo.ContainerServer) *cmdInitBridgeParams {
	...
	for {
		bridge.Name = cmd.Context.AskString("What should the new bridge be called [default=apollobr0]? ", "apollobr0", networkValidName)
		...
		if !shared.StringInSlice(bridge.IPv4, []string{"auto", "none"}) {
			bridge.IPv4Nat = cmd.Context.AskBool("Would you like APOLLO to NAT IPv4 traffic on your bridge? [default=yes]? ", "yes")
		}
		...
		if !shared.StringInSlice(bridge.IPv6, []string{"auto", "none"}) {
			bridge.IPv6Nat = cmd.Context.AskBool("Would you like APOLLO to NAT IPv6 traffic on your bridge? [default=yes]? ", "yes")
		}
		break
	}
	return bridge
...
// apollo init command, either directly fed via --preseed or populated by
...
// Parameters needed when configuring the APOLLO server networking options in interactive
// mode or auto mode.
