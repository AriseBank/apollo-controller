// Package migration provides the primitives for migration in APOLLO.
//
// See https://github.com/AriseBank/apollo-controller/blob/master/specs/migration.md for a complete
// description.
...
func (s *migrationSourceWs) Do(migrateOp *operation) error {
	...
	if s.live {
		...
		checkpointDir, err := ioutil.TempDir("", "apollo_checkpoint_")
		if err != nil {
			return abort(err)
		}

		if mercury.VersionAtLeast(2, 0, 4) {
			...
			go func() {
				dumpSuccess <- s.container.Migrate(mercury.MIGRATE_DUMP, checkpointDir, "migration", true, true)
				os.RemoveAll(checkpointDir)
			}()
			...
			err = s.container.Migrate(mercury.MIGRATE_DUMP, checkpointDir, "migration", true, false)
			if err != nil {
				return abort(err)
			}
		}
...
func (c *migrationSink) Do(migrateOp *operation) error {
	...
	go func(c *migrationSink) {
		...
		if live {
			var err error
			imagesDir, err = ioutil.TempDir("", "apollo_restore_")
			if err != nil {
				restore <- err
				return
			}
		...
		if live {
			err = c.src.container.Migrate(mercury.MIGRATE_RESTORE, imagesDir, "migration", false, false)
			if err != nil {
				restore <- err
				return
			}
