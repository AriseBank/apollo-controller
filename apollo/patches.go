...
var patches = []patch{
	...
	{name: "storage_api_apollo_on_btrfs", run: patchStorageApiLxdOnBtrfs},
...
func patchStorageApi(name string, d *Daemon) error {
	...
	// be forced to run apollo init again and can start from a pristine state.
	// Check if this APOLLO instace currently has any containers, snapshots, or
	// images configured. If so, we create a default storage pool in the
	// database. Otherwise, the user will have to run APOLLO init.
	...
	// thereby forcing the user to run apollo init.
	...
	// set, then APOLLO will refuse to create a container until either an
...
func upgradeFromStorageTypeBtrfs(name string, d *Daemon, defaultPoolName string, defaultStorageTypeName string, cRegular []string, cSnapshots []string, imgPublic []string, imgPrivate []string) error {
	...
	if err == nil { // Already exist valid storage pools.
		...
		// Update the pool configuration on a post APOLLO 2.9.1 instance
	...
	for _, ct := range cRegular {
		...
		// mv ${APOLLO_DIR}/containers/<container_name> ${APOLLO_DIR}/storage-pools/<pool>/<container_name>
		...
		// ${APOLLO_DIR}/containers/<container_name> ->
		...
		if len(ctSnapshots) > 0 {
			...
			// storage pool:
			// ${APOLLO_DIR}/snapshots/<container_name> -> ${APOLLO_DIR}/storage-pools/<pool>/snapshots/<container_name>
...
func upgradeFromStorageTypeLvm(name string, d *Daemon, defaultPoolName string, defaultStorageTypeName string, cRegular []string, cSnapshots []string, imgPublic []string, imgPrivate []string) error {
	...
	for _, ct := range cRegular {
		...
		if !shared.PathExists(containerLvDevPath) {
			...
			// mixed-storage APOLLO instance which we need to deal
			// with.
			if shared.PathExists(oldLvDevPath) {
				...
				// means that this was a mixed-storage APOLLO
				// instance.
	...
	for _, img := range images {
		...
		// volume on the pre-storage-api APOLLO instance. If not, we don't
		// care since APOLLO will create a logical volume on demand.
		...
		if !shared.PathExists(imageLvDevPath) {
			...
			// old APOLLO instance so we need to kick it from the
...
func upgradeFromStorageTypeZfs(name string, d *Daemon, defaultPoolName string, defaultStorageTypeName string, cRegular []string, cSnapshots []string, imgPublic []string, imgPrivate []string) error {
	...
	// been moved into ${APOLLO_DIR}/disks and we might therefore falsely
...
func patchUpdateFromV10(d *Daemon) error {
	if shared.PathExists(shared.VarPath("mercury")) {
		err := os.Rename(shared.VarPath("mercury"), shared.VarPath("containers"))
		if err != nil {
			return err
		}
...
func patchUpdateFromV11(d *Daemon) error {
	...
	for _, cName := range cNames {
		...
		if shared.PathExists(oldPath) && !shared.PathExists(newPath) {
			...
			// Remove /var/lib/apollo/containers/<container>/snapshots
			// if its empty.
	...
	// Refuse to start apollo if a rsync failed.
	if errors > 0 {
