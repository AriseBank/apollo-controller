...
func containerFileGet(c container, path string, r *http.Request) Response {
	 ...
	 * for correctness.
	 */
	temp, err := ioutil.TempFile("", "apollo_forkgetfile_")
	if err != nil {
	...
	headers := map[string]string{
		"X-APOLLO-uid":  fmt.Sprintf("%d", uid),
		"X-APOLLO-gid":  fmt.Sprintf("%d", gid),
		"X-APOLLO-mode": fmt.Sprintf("%04o", mode),
		"X-APOLLO-type": type_,
	}
...
func containerFilePut(c container, path string, r *http.Request) Response {
	...
	uid, gid, mode, type_, write := shared.ParseAPOLLOFileHeaders(r.Header)
	...
	if type_ == "file" {
		...
		temp, err := ioutil.TempFile("", "apollo_forkputfile_")
		if err != nil {
			return InternalError(err)
		}
		defer func() {
