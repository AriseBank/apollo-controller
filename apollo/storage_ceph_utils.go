...
// state
// An RBD storage volume that is in zombie state is not tracked in APOLLO's
...
// creating a sparse copy of a container or when APOLLO updated an image and the
...
// state
// - An RBD storage volume that is in zombie is not tracked in APOLLO's database
...
//   APOLLO detects that the storage volume for the given hash already exists in
...
// helper (currently named "apolloResolveMountoptions") which will take on the job
...
func cephContainerDelete(clusterName string, poolName string, volumeName string,
	...
	if err == nil {
		...
		if err == nil {
			...
			// it is a zombie. If it is not we know that APOLLO is
			// still using it.
...
func cephContainerSnapshotDelete(clusterName string, poolName string,
	...
	if err != nil {
		...
		// we know that APOLLO is still using it.
		if strings.HasPrefix(volumeType, "zombie_") {
		...
		if canDelete {
			...
			// is not we know that APOLLO is still using it.
			if strings.HasPrefix(volumeType, "zombie_") {
...
// <osd-pool-name>/<apollo-specific-prefix>_<rbd-storage-volume>@<rbd-snapshot-name>
// and will split it into
// <osd-pool-name>, <rbd-storage-volume>, <apollo-specific-prefix>, <rbdd-snapshot-name>
...
// <osd-pool-name>/<apollo-specific-prefix>_<rbd-storage-volume>
// will be split into
// <osd-pool-name>, <apollo-specific-prefix>, <rbd-storage-volume>
