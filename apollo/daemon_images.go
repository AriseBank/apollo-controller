...
func imageLoadStreamCache(d *Daemon) error {
	...
	for url, entry := range imageStreamCache {
		if entry.remote == nil {
			remote, err := apollo.ConnectSimpleStreams(url, &apollo.ConnectionArgs{
...
func (d *Daemon) ImageDownload(op *operation, server string, protocol string, certificate string, secret string, alias string, forContainer bool, autoUpdate bool, storagePool string, preferCached bool) (*api.Image, error) {
	...
	var remote apollo.ImageServer
	var info *api.Image

	// Default protocol is APOLLO
	if protocol == "" {
		protocol = "apollo"
	}
	...
	if protocol == "simplestreams" {
		...
		if entry == nil || entry.expiry.Before(time.Now()) {
			...
			refresh := func() (*imageStreamCacheEntry, error) {
				...
				remote, err = apollo.ConnectSimpleStreams(server, &apollo.ConnectionArgs{
	...
	} else if protocol == "apollo" {
		// Setup APOLLO client
		remote, err = apollo.ConnectPublicAPOLLO(server, &apollo.ConnectionArgs{
			TLSServerCert: certificate,
			UserAgent:     version.UserAgent,
			Proxy:         d.proxy,
		})
	...
	progress := func(progress apollo.ProgressData) {
		if op == nil {
			return
		}
	...
	if protocol == "apollo" || protocol == "simplestreams" {
		...
		// Download the image
		var resp *apollo.ImageFileResponse
		request := apollo.ImageFileRequest{
		...
		body := &ioprogress.ProgressReader{
			...
			Tracker: &ioprogress.ProgressTracker{
				...
				Handler: func(percent int64, speed int64) {
					progress(apollo.ProgressData{Text: fmt.Sprintf("%d%% (%s/s)", percent, shared.GetByteSizeString(speed, 2))})
				},
			},
		}
