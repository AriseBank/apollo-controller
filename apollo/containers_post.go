...
func createFromMigration(d *Daemon, req *api.ContainersPost) Response {
	...
	// Handle copying/moving between two storage-api APOLLO instances.
	if storagePool != "" {
	...
	if storagePool == "" {
		...
		if err != nil {
			if err == db.NoSuchObjectError {
				return BadRequest(fmt.Errorf("This APOLLO instance does not have any storage pools configured."))
			}
			return SmartError(err)
		}
	...
	if err != nil {
		...
		cM, err := containerMERCURYLoad(d, args)
		if err != nil {
			return InternalError(err)
		}
