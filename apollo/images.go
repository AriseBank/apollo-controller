...
func imgPostContInfo(d *Daemon, r *http.Request, req api.ImagesPost, builddir string) (*api.Image, error) {
	...
	tarfile, err := ioutil.TempFile(builddir, "apollo_build_tar_")
	if err != nil {
		return nil, err
	}
...
func imgPostURLInfo(d *Daemon, req api.ImagesPost, op *operation) (*api.Image, error) {
	...
	head.Header.Set("APOLLO-Server-Architectures", strings.Join(architecturesStr, ", "))
	head.Header.Set("APOLLO-Server-Version", version.Version)
	...
	hash := raw.Header.Get("APOLLO-Image-Hash")
	if hash == "" {
		return nil, fmt.Errorf("Missing APOLLO-Image-Hash header")
	}

	url := raw.Header.Get("APOLLO-Image-URL")
	if url == "" {
		return nil, fmt.Errorf("Missing APOLLO-Image-URL header")
	}
...
func getImgPostInfo(d *Daemon, r *http.Request, builddir string, post *os.File) (*api.Image, error) {
	...
	public, _ := strconv.Atoi(r.Header.Get("X-APOLLO-public"))
	info.Public = public == 1
	propHeaders := r.Header[http.CanonicalHeaderKey("X-APOLLO-properties")]
	...
	if ctype == "multipart/form-data" {
		...
		imageTarf, err := ioutil.TempFile(builddir, "apollo_tar_")
		if err != nil {
			return nil, err
		}
		...
		rootfsTarf, err := ioutil.TempFile(builddir, "apollo_tar_")
		if err != nil {
			return nil, err
		}
		...
		expectedFingerprint := r.Header.Get("X-APOLLO-fingerprint")
		...
		info.Filename = r.Header.Get("X-APOLLO-filename")
		info.Fingerprint = fmt.Sprintf("%x", sha256.Sum(nil))
...
func imagesPost(d *Daemon, r *http.Request) Response {
	...
	builddir, err := ioutil.TempDir(shared.VarPath("images"), "apollo_build_")
	if err != nil {
	...
	post, err := ioutil.TempFile(builddir, "apollo_post_")
	if err != nil {
