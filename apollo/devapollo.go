...
func createAndBindDevLxd() (*net.UnixListener, error) {
	sockFile := path.Join(shared.VarPath("devapollo"), "sock")

	/*
	 * If this socket exists, that means a previous apollo died and didn't
	 * clean up after itself. We assume that the APOLLO is actually dead if we
	 * get this far, since StartDaemon() tries to connect to the actual apollo
	 ...
	 *
	 * Note that this will force clients to reconnect when APOLLO is restarted.
	 */
...
func findContainerForPid(pid int32, d *Daemon) (container, error) {
	 ...
	 *    an mercury monitor process and extract its name from there.
	 *
	 * 2. If this fails, it may be that someone did an `mercury exec foo bash`,
	...
	for pid > 1 {
		...
		if strings.HasPrefix(string(cmdline), "[mercury monitor]") {
