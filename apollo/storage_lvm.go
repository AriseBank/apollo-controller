...
func (s *storageLvm) StoragePoolCreate() error {
	...
	if vgExisted {
		...
		inUse, user, err := apolloUsesPool(s.d.db, poolName, s.pool.Driver, "lvm.vg_name")
		if err != nil {
			return err
		}

		if inUse {
			msg := fmt.Sprintf("APOLLO already uses volume group \"%s\" for pool \"%s\"", poolName, user)
			logger.Errorf(msg)
			return fmt.Errorf(msg)
		}
	} else {
...
func (s *storageLvm) StoragePoolVolumeMount() (bool, error) {
	...
	if !shared.IsMountPoint(customPoolVolumeMntPoint) {
		mountFlags, mountOptions := apolloResolveMountoptions(s.getLvmMountOptions())
