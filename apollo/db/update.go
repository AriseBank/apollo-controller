...
/* Database updates are one-time actions that are needed to move an
   ...
   Those updates are applied at startup time before anything else in APOLLO
   is initialized. This means that they should be entirely
   self-contained and not touch anything but the database.

   Calling APOLLO functions isn't allowed as such functions may themselves
   ...
   changes to the database schema. Use patches instead (see apollo/patches.go).
...
func UpdatesApplyAll(db *sql.DB, doBackup bool, postApply func(int) error) error {
	...
	for _, update := range dbUpdates {
		...
		if doBackup && !backup {
			logger.Infof("Updating the APOLLO database schema. Backup made as \"apollo.db.bak\"")
			err := shared.FileCopy(shared.VarPath("apollo.db"), shared.VarPath("apollo.db.bak"))
			if err != nil {
				return err
			}
...
func dbUpdateFromV20(currentVersion int, version int, db *sql.DB) error {
	stmt := `
UPDATE containers_devices SET name='__apollo_upgrade_root' WHERE name='root';
UPDATE profiles_devices SET name='__apollo_upgrade_root' WHERE name='root';
