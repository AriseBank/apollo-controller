...
func (suite *containerTestSuite) TestContainer_LoadFromDB() {
	...
	// Load the container and trigger initMERCURY()
...
func (suite *containerTestSuite) TestContainer_findIdmap_isolated() {
	...
	map1, err := c1.(*containerMERCURY).NextIdmapSet()
	suite.Req.Nil(err)
	map2, err := c2.(*containerMERCURY).NextIdmapSet()
	suite.Req.Nil(err)
...
func (suite *containerTestSuite) TestContainer_findIdmap_maxed() {
	...
	for i := 0; i < 7; i++ {
		...
		m, err := c.(*containerMERCURY).NextIdmapSet()
		suite.Req.Nil(err)
