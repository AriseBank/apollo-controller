...
func (s *storageCeph) MigrationSource(c container, containerOnly bool) (MigrationStorageSourceDriver, error) {
	...
	for _, snap := range snapshots {
		...
		apolloName := fmt.Sprintf("%s%s%s", containerName, shared.SnapshotDelimiter, snap[len("snapshot_"):])
		snapshot, err := containerLoadByName(s.d, apolloName)
		if err != nil {
			logger.Errorf(`Failed to load snapshot "%s" for RBD storage volume "%s" on storage pool "%s": %s`, apolloName, containerName, s.pool.Name, err)
			return nil, err
		}
...
func (s *storageCeph) MigrationSink(live bool, c container, snapshots []*Snapshot, conn *websocket.Conn, srcIdmap *shared.IdmapSet, op *operation, containerOnly bool) error {
	...
	// TODO: The cluster name can be different between APOLLO instances. Find
	...
	// the receiving APOLLO instance it also means that s.ClusterName has been
	// set to the correct cluster name for that APOLLO instance. Yeah, I think
