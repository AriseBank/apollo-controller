...
func (s *storageZfs) StoragePoolVolumeUmount() (bool, error) {
	...
	apolloStorageMapLock.Lock()
	if waitChannel, ok := apolloStorageOngoingOperationMap[customUmountLockID]; ok {
		apolloStorageMapLock.Unlock()
		if _, ok := <-waitChannel; ok {
			logger.Warnf("Received value over semaphore. This should not have happened.")
		}
	...
	apolloStorageOngoingOperationMap[customUmountLockID] = make(chan bool)
	apolloStorageMapLock.Unlock()
	...
	if waitChannel, ok := apolloStorageOngoingOperationMap[customUmountLockID]; ok {
		close(waitChannel)
		delete(apolloStorageOngoingOperationMap, customUmountLockID)
	}
	apolloStorageMapLock.Unlock()
...
// - create temporary directory ${APOLLO_DIR}/images/apollo_images_
// - create new zfs volume images/<fingerprint>
// - mount the zfs volume on ${APOLLO_DIR}/images/apollo_images_
// - unpack the downloaded image in ${APOLLO_DIR}/images/apollo_images_
...
func (s *storageZfs) ImageCreate(fingerprint string) error {
	...
	if zfsFilesystemEntityExists(poolName, fmt.Sprintf("deleted/%s", fs)) {
		...
		// In case this is an image from an older apollo instance, wipe the
		// mountpoint.
		err = zfsPoolVolumeSet(poolName, fs, "mountpoint", "none")
		if err != nil {
			return err
	...
	* xfer costs. Then, after all that, we send the container itself.
	...
	for _, snap := range snapshots {
		...
		* will take too long to delete.)
		...
		apolloName := fmt.Sprintf("%s%s%s", ct.Name(), shared.SnapshotDelimiter, snap[len("snapshot-"):])
		snapshot, err := containerLoadByName(s.d, apolloName)
		if err != nil {
			return nil, err
		}
