...
func (s *storageDir) StoragePoolCreate() error {
	...
	if source == "" {
		...
		if strings.HasPrefix(cleanSource, apolloDir) && cleanSource != poolMntPoint {
			return fmt.Errorf("DIR storage pool requests in APOLLO directory \"%s\" are only valid under \"%s\"\n(e.g. source=%s)", shared.VarPath(), shared.VarPath("storage-pools"), poolMntPoint)
		}
	}
...
func (s *storageDir) ContainerRename(container container, newName string) error {
	...
	// ${APOLLO_DIR}/snapshots/<old_container_name>
	...
	if shared.PathExists(oldSnapshotSymlink) {
		...
		// ${APOLLO_DIR}/snapshots/<new_container_name> -> ${POOL}/snapshots/<new_container_name>
...
onSuccess:
	...
	// ${APOLLO_DIR}/snapshots/<source_container_name> -> ${POOL_PATH}/snapshots/<source_container_name>
