...
import (
	...
	"github.com/AriseBank/apollo-controller/shared/logging"
...
func (d *Daemon) createCmd(version string, c Command) {
	...
	d.mux.HandleFunc(uri, func(w http.ResponseWriter, r *http.Request) {
		...
		/*
		 * When we create a new mercury.Container, it adds a finalizer (via
...
func (d *Daemon) SetupStorageDriver(forceCheck bool) error {
	...
	// on this APOLLO instance. Only do this when we do not already have done
...
func (d *Daemon) UpdateHTTPsPort(newAddress string) error {
	...
	if newAddress != "" {
		...
		d.tomb.Go(func() error { return http.Serve(tcpl, &apolloHttpServer{d.mux, d}) })
...
func (d *Daemon) Init() error {
	...
	if d.MockMode {
		logger.Info(fmt.Sprintf("APOLLO %s is starting in mock mode", version.Version),
			log.Ctx{"path": shared.VarPath("")})
	} else if d.SetupMode {
		logger.Info(fmt.Sprintf("APOLLO %s is starting in setup mode", version.Version),
			log.Ctx{"path": shared.VarPath("")})
	} else {
		logger.Info(fmt.Sprintf("APOLLO %s is starting in normal mode", version.Version),
			log.Ctx{"path": shared.VarPath("")})
	}
	...
	if os.Getenv("APOLLO_SECURITY_APPARMOR") == "false" {
	...
	if !haveMacAdmin() {
		...
		if aaAvailable {
			logger.Warnf("Per-container AppArmor profiles are disabled because APOLLO is running in an unprivileged container without stacking.")
		}
	} else {
		aaAdmin = true
	}
	...
	if profile != "unconfined" && profile != "" {
		if aaAvailable {
			logger.Warnf("Per-container AppArmor profiles are disabled because APOLLO is already protected by AppArmor.")
		}
		aaConfined = true
	}
	...
	/* Set container path */
	d.mercurypath = shared.VarPath("containers")
	...
	if err := os.MkdirAll(shared.VarPath("devapollo"), 0755); err != nil {
		return err
	}
	...
	d.BackingFs, err = filesystemDetect(d.mercurypath)
	if err != nil {
	...
	if err != nil {
		...
		if err == nil {
			...
			for _, mercurymap := range kernelIdmapSet.ToLxcString() {
				logger.Infof(strings.TrimRight(" - "+mercurymap, "\n"))
			}
		}
		...
		if len(d.IdmapSet.Idmap) == 0 {
			...
			logger.Infof("Configured APOLLO uid/gid map:")
			for _, mercurymap := range d.IdmapSet.Idmap {
				suffix := ""

				if mercurymap.Usable() != nil {
					suffix = " (unusable)"
				}

				for _, mercuryEntry := range mercurymap.ToLxcString() {
					logger.Infof(" - %s%s", strings.TrimRight(mercuryEntry, "\n"), suffix)
				}
			}
	...
	if !d.MockMode {
		...
		// Attempt to Mount the devapollo tmpfs
		if !shared.IsMountPoint(shared.VarPath("devapollo")) {
			syscall.Mount("tmpfs", shared.VarPath("devapollo"), "tmpfs", 0, "size=100k,mode=0755")
		}
	}

	/* Setup /dev/apollo */
	logger.Infof("Starting /dev/apollo handler")
	d.devapollo, err = createAndBindDevLxd()
	if err != nil {
		return err
	}
	...
	d.tomb.Go(func() error {
		...
		return server.Serve(d.devapollo)
	})
	...
	if !d.MockMode {
		...
		if shared.PathExists(shared.VarPath("server.ca")) {
			...
			logger.Infof("APOLLO is in CA mode, only CA-signed certificates will be allowed")
		}
	...
	if len(listeners) > 0 {
		logger.Infof("APOLLO is socket activated")
		...
		logger.Infof("APOLLO isn't socket activated")
		...
		if shared.PathExists(localSocketPath) {
			...
			if err != nil {
				...
				return fmt.Errorf("APOLLO is already running.")
			}
		}
	...
	if d.UnixSocket != nil {
		...
		d.tomb.Go(func() error { return http.Serve(d.UnixSocket.Socket, &apolloHttpServer{d.mux, d}) })
	}

	if d.TCPSocket != nil {
		logger.Info(" - binding TCP socket", log.Ctx{"socket": d.TCPSocket.Socket.Addr()})
		d.tomb.Go(func() error { return http.Serve(d.TCPSocket.Socket, &apolloHttpServer{d.mux, d}) })
	}
...
func (d *Daemon) Ready() error {
	...
	/* Re-balance in case things changed while APOLLO was down */
	deviceTaskBalance(d)
...
func (d *Daemon) Stop() error {
	...
	logger.Infof("Stopping /dev/apollo handler")
	d.devapollo.Close()
	logger.Infof("Stopped /dev/apollo handler")

	if n, err := d.numRunningContainers(); err != nil || n == 0 {
		logger.Infof("Unmounting temporary filesystems")

		syscall.Unmount(shared.VarPath("devapollo"), syscall.MNT_DETACH)
	...
	for _, entry := range entries {
		...
		if shared.StringInSlice(entry.Name(), result) {
			...
			for _, logfile := range logs {
				...
				// Always keep the MERCURY config
				if logfile.Name() == "mercury.conf" {
					continue
				}
...
type apolloHttpServer struct {
	r *mux.Router
	d *Daemon
}

func (s *apolloHttpServer) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
...
func initializeDbObject(d *Daemon, path string) error {
	...
	// Detect APOLLO downgrades
	if db.GetSchema(d.db) > db.GetLatestSchema() {
		return fmt.Errorf("The database schema is more recent than APOLLO's schema.")
	}
	...
	// patches mechanism was introduced in apollo/patches.go. The
