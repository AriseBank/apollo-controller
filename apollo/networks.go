...
func networkStartup(d *Daemon) error {
	...
	for _, name := range networks {
		...
		err = n.Start()
		if err != nil {
			// Don't cause APOLLO to fail to start entirely on network bring up failure
...
func (n *network) Start() error {
	...
	if n.config["bridge.mode"] == "fan" || !shared.StringInSlice(n.config["ipv4.address"], []string{"", "none"}) || !shared.StringInSlice(n.config["ipv6.address"], []string{"", "none"}) {
		...
		for _, user := range []string{"apollo", "nobody"} {
		...
		if err != nil {
			return fmt.Errorf("dnsmasq is required for APOLLO managed bridges.")
		}
