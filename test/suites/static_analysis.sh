safe_pot_hash() {
  sed -e "/Project-Id-Version/,/Content-Transfer-Encoding/d" -e "/^#/d" "po/apollo.pot" | md5sum | cut -f1 -d" "
}
...
test_static_analysis() {
  (
    ...
    if which golint >/dev/null 2>&1; then
      ...
      golint -set_exit_status mercury/config/
    ...
    if which deadcode >/dev/null 2>&1; then
      OUT=$(deadcode ./fuidshift ./mercury ./apollo ./apollo/types ./shared ./shared/api ./shared/i18n ./shared/ioprogress ./shared/logging ./shared/osarch ./shared/simplestreams ./shared/termios ./shared/version ./test/apollo-benchmark 2>&1 | grep -v apollo/migrate.pb.go: | grep -v /C: | grep -vi _cgo | grep -vi _cfunc || true)
      if [ -n "${OUT}" ]; then
        echo "${OUT}" >&2
        false
      fi
    ...
    cp --preserve "po/apollo.pot" "po/apollo.pot.bak"
    hash1=$(safe_pot_hash)
    make i18n -s
    hash2=$(safe_pot_hash)
    mv "po/apollo.pot.bak" "po/apollo.pot"
    if [ "${hash1}" != "${hash2}" ]; then
