# Introduction
All the communications between APOLLO and its clients happen using a
...
# Status codes
The APOLLO REST API often has to return status information, be that the
...
# PUT vs PATCH
The APOLLO API supports both PUT and PATCH to modify existing objects.
...
response and sent as If-Match for the PUT request. This will cause APOLLO
...
Input (using a public remote image):

    {
        ...
        "source": {"type": "image",                                         # Can be: "image", "migration", "copy" or "none"
                   ...
                   "protocol": "apollo",                                       # Protocol (one of apollo or simplestreams, defaults to apollo)
...
Input (migration across apollo instances):
...
The migration does not actually start until someone (i.e. another apollo instance)
 ...
 * X-APOLLO-gid: 0
 * X-APOLLO-mode: 0700
 * X-APOLLO-type: one of "directory" or "file"
 ...
 * X-APOLLO-gid: 0
 * X-APOLLO-mode: 0700
 * X-APOLLO-type: one of "directory", "file" or "symlink"
 * X-APOLLO-write: overwrite (or append, introduced with API extension "file\_append")
...
Return:
    ...
    [
        ...
        "/1.0/containers/blah/logs/mercury.conf",
        "/1.0/containers/blah/logs/mercury.log"
    ]
 ...
 * X-APOLLO-fingerprint: SHA-256 (if set, uploaded file must match)
 * X-APOLLO-filename: FILENAME (used for export)
 * X-APOLLO-public: true/false (defaults to false)
 * X-APOLLO-properties: URL-encoded key value pairs without duplicate keys (optional properties)
...
After the input is received by APOLLO, a background operation is started
...
The secret string is required when an untrusted APOLLO is spawning a new
container from a private image stored on a different APOLLO.

Rather than require a trust relationship between the two APOLLOs, the
client will POST to /1.0/images/\<fingerprint\>/export to get a secret
token which it'll then pass to the target APOLLO. That target APOLLO will then
...
### POST
 * Description: Generate a random token and tell APOLLO to expect it be used by a guest
 * Authentication: guest or trusted
 * Operation: async
 * Return: background operation or standard error
