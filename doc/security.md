...
Remote communications with the APOLLO daemon happen using JSON over HTTPS.
...
the APOLLO socket and the client will use its certificate as a client
...
In the default setup, when the user adds a new server with `mercury remote add`,
...
PKI then issues client certificates for all the mercury clients and server
certificates for all the APOLLO daemons.
...
In that mode, any connection to a APOLLO daemon will be done using the
...
client's configuration directory (~/.config/mercury) and a server.ca file in
the server's configuration directory (/var/lib/apollo). Then a client
...
The list of certificates trusted by a APOLLO server can be obtained with `mercury
config trust list`.

To revoke trust to a client its certificate can be removed with `mercury config
...
rules should be set to only allow access to the APOLLO port from authorized
hosts/subnets.
