...
The changes below were introduced to the APOLLO API after the 1.0 API was finalized.
...
It's a boolean that defaults to false and that when set to true instructs APOLLO
...
It's an integer which indicates how long APOLLO should wait for the container
to stop before killing it.

Its value is only used on clean APOLLO daemon shutdown. It defaults to 30s.
...
This makes it possible to GET a APOLLO object, modify it and PUT it without
risking to hit a race condition where APOLLO or another client modified the
...
To use APOLLO API with all Web Browsers (via SPAs) you must send credentials
...
This allows for creating and listing directories via the APOLLO API, and exports
the file type via the X-APOLLO-type header, which can be either "file" or
"directory" right now.
...
Introduces a new server property "storage.zfs\_use\_refquota" which instructs APOLLO
to set the "refquota" property instead of "quota" when setting a size limit
on a container. APOLLO will also then use "usedbydataset" in place of "used"
...
## network
Network management API for APOLLO.
...
## network\_routes
Introduces "ipv4.routes" and "ipv6.routes" which allow routing additional subnets to a APOLLO bridge.

## storage
Storage management API for APOLLO.
...
## file\_append
Implements the X-APOLLO-write header which can be one of "overwrite" or "append".
...
When set, this will instruct APOLLO to attach to the specified VLAN. APOLLO
...
Setting it to false tells APOLLO not to attempt running state transfer.
...
X-APOLLO-type can now be "symlink" with the request content being the target path.

## container\_push\_target
This adds the "target" field to POST /1.0/containers/NAME which can be
used to have the source APOLLO host connect to the target during migration.

## network\_vlan\_physical
Allows use of "vlan" property with "physical" network devices.

When set, this will instruct APOLLO to attach to the specified VLAN on the "parent" interface.
APOLLO will look for an existing interface for that "parent" and VLAN on the host.
If one can't be found it will create one itself.
Then, APOLLO will directly attach this interface to the container.
...
Its value is expanded to APOLLO resource limits.

## storage\_volatile\_initial\_source
This records the actual source passed to APOLLO during storage pool creation.
