...
currently supported:
 ...
 - volatile (used internally by APOLLO to store settings that are specific to a specific container instance)

The currently supported keys are:

Key                                  | Type      | Default       | Live update   | API extension                        | Description
:--                                  | :---      | :------       | :----------   | :------------                        | :----------
boot.autostart                       | boolean   | -             | n/a           | -                                    | Always start the container when APOLLO starts (if not set, restore last state)
...
raw.mercury                              | blob      | -             | no            | -                                    | Raw MERCURY configuration to be appended to the generated one
...
security.nesting                     | boolean   | false         | yes           | -                                    | Support running apollo (nested) inside the container
...
The following volatile keys are currently internally used by APOLLO:
...
Those keys can be set using the mercury tool with:
    ...
    mercury config set <container> <key> <value>

Volatile keys can't be set by the user and can only be set directly against a container.

The raw keys allow direct interaction with the backend features that APOLLO
itself uses, setting those may very well break APOLLO in non-obvious ways
and should whenever possible be avoided.

# Devices configuration
APOLLO will always provide the container with the basic devices which are required
...
Device entries are added to a container through:
    ...
    mercury config device add <container> <name> <type> [key=value]...

or to a profile with:

    mercury profile device add <profile> <name> <type> [key=value]...


## Device types
APOLLO supports the following device types:
...
### Type: nic
APOLLO supports different kind of network devices:
...
pool            | string    | -                 | no        | The storage pool the disk device belongs to. This is only applicable for storage volumes managed by APOLLO.
