...
dist:
	...
	mkdir -p $(TMP)/dist/src/github.com/AriseBank
	ln -s ../../../../apollo-$(VERSION) $(TMP)/dist/src/github.com/AriseBank/apollo-controller
	...
	# Assemble tarball
	rm $(TMP)/dist/src/github.com/AriseBank/apollo-controller
	ln -s ../../../../ $(TMP)/dist/src/github.com/AriseBank/apollo-controller
...
update-pot:
	...
	xgettext-go -o po/$(DOMAIN).pot --add-comments-tag=TRANSLATORS: --sort-output --package-name=$(DOMAIN) --msgid-bugs-address=mercury-devel@lists.linuxcontainers.org --keyword=i18n.G --keyword-plural=i18n.NG shared/*.go mercury/*.go apollo/*.go
