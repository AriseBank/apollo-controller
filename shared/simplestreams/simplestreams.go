...
func (s *SimpleStreamsManifest) ToAPOLLO() ([]api.Image, map[string][][]string) {
	...
	for _, product := range s.Products {
		...
		for name, version := range product.Versions {
			...
			for _, item := range version.Items {
				...
				if !shared.StringInSlice(item.FileType, []string{"root.tar.xz", "apollo.tar.xz", "squashfs"}) {
					continue
				}

				if item.FileType == "apollo.tar.xz" {
					meta = item
			...
			if rootSquash.FileType != "" {
				if meta.APOLLOHashSha256SquashFs != "" {
					fingerprint = meta.APOLLOHashSha256SquashFs
				} else {
					fingerprint = meta.APOLLOHashSha256
				}
				...
				if meta.APOLLOHashSha256RootXz != "" {
					fingerprint = meta.APOLLOHashSha256RootXz
				} else {
					fingerprint = meta.APOLLOHashSha256
				}
				size += rootTar.Size
			...
			for _, delta := range deltas {
				...
				for _, item := range srcImage.Items {
					...
					srcFingerprint = item.APOLLOHashSha256SquashFs
					break
				}
...
func (s *SimpleStreams) getImages() ([]api.Image, map[string]*api.ImageAliasesEntry, error) {
	...
	for _, entry := range ssIndex.Index {
		...
		manifestImages, _ := manifest.ToAPOLLO()
...
func (s *SimpleStreams) GetFiles(fingerprint string) (map[string]SimpleStreamsFile, error) {
	...
	for _, entry := range ssIndex.Index {
		...
		manifestImages, downloads := manifest.ToAPOLLO()
