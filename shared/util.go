...
// appended to the end of $APOLLO_DIR, which defaults to /var/lib/apollo.
func VarPath(path ...string) string {
	varDir := os.Getenv("APOLLO_DIR")
	if varDir == "" {
		varDir = "/var/lib/apollo"
	}
...
// CachePath returns the directory that APOLLO should its cache under. If APOLLO_DIR is
// set, this path is $APOLLO_DIR/cache, otherwise it is /var/cache/apollo.
func CachePath(path ...string) string {
	varDir := os.Getenv("APOLLO_DIR")
	logDir := "/var/cache/apollo"
	if varDir != "" {
...
// LogPath returns the directory that APOLLO should put logs under. If APOLLO_DIR is
// set, this path is $APOLLO_DIR/logs, otherwise it is /var/log/apollo.
func LogPath(path ...string) string {
	varDir := os.Getenv("APOLLO_DIR")
	logDir := "/var/log/apollo"
	if varDir != "" {
...
func ParseAPOLLOFileHeaders(headers http.Header) (uid int64, gid int64, mode int, type_ string, write string) {
	uid, err := strconv.ParseInt(headers.Get("X-APOLLO-uid"), 10, 64)
	if err != nil {
		uid = -1
	}

	gid, err = strconv.ParseInt(headers.Get("X-APOLLO-gid"), 10, 64)
	if err != nil {
		gid = -1
	}

	mode, err = strconv.Atoi(headers.Get("X-APOLLO-mode"))
	if err != nil {
		mode = -1
	} else {
		rawMode, err := strconv.ParseInt(headers.Get("X-APOLLO-mode"), 0, 0)
		if err == nil {
			mode = int(os.FileMode(rawMode) & os.ModePerm)
		}
	}

	type_ = headers.Get("X-APOLLO-type")
	...
	write = headers.Get("X-APOLLO-write")
...
func TextEditor(inPath string, inContent []byte) ([]byte, error) {
	...
	if inPath == "" {
		...
		f, err = ioutil.TempFile("", "apollo_editor_")
		if err != nil {
			return []byte{}, err
		}
...
func ParseByteSizeString(input string) (int64, error) {
	...
	if unicode.IsNumber(rune(input[len(input)-1])) {
		...
		// Backward compatible behaviour in case we talk to a APOLLO that
