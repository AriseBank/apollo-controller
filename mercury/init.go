...
func (c *initCmd) usage() string {
	return i18n.G(
		`Usage: mercury init [<remote>:]<image> [<remote>:][<name>] [--ephemeral|-e] [--profile|-p <profile>...] [--config|-c <key=value>...] [--network|-n <network>] [--storage|-s <pool>] [--type|-t <instance type>]
...
func (c *initCmd) create(conf *config.Config, args []string) (apollo.ContainerServer, string, error) {
	...
	var imgRemote apollo.ImageServer
...
func (c *initCmd) guessImage(conf *config.Config, d apollo.ContainerServer, remote string, iremote string, image string) (string, string) {
	if remote != iremote {
...
func (c *initCmd) checkNetwork(d apollo.ContainerServer, name string) {
	...
	fmt.Fprintf(os.Stderr, "  "+i18n.G("To create a new network, use: mercury network create")+"\n")
	fmt.Fprintf(os.Stderr, "  "+i18n.G("To attach a network to a container, use: mercury network attach")+"\n\n")
}
