...
func (c *fileCmd) usage() string {
	return i18n.G(
		`Usage: mercury file <subcommand> [options]

Manage files in containers.

mercury file pull [-r|--recursive] [<remote>:]<container>/<path> [[<remote>:]<container>/<path>...] <target path>
    Pull files from containers.

mercury file push [-r|--recursive] [-p|--create-dirs] [--uid=UID] [--gid=GID] [--mode=MODE] <source path> [<source path>...] [<remote>:]<container>/<path>
    Push files into containers.

mercury file delete [<remote>:]<container>/<path> [[<remote>:]<container>/<path>...]
    Delete files in containers.

mercury file edit [<remote>:]<container>/<path>
    Edit files in containers using the default text editor.

*Examples*
mercury file push /etc/hosts foo/etc/hosts
   To push /etc/hosts into the container "foo".

mercury file pull foo/etc/hosts .
...
func (c *fileCmd) recursivePullFile(d apollo.ContainerServer, container string, p string, targetDir string) error {
...
func (c *fileCmd) recursivePushFile(d apollo.ContainerServer, container string, source string, target string) error {
...
func (c *fileCmd) recursiveMkdir(d apollo.ContainerServer, container string, p string, mode *os.FileMode, uid int64, gid int64) error {
...
func (c *fileCmd) edit(conf *config.Config, args []string) error {
	...
	// Create temp file
	f, err := ioutil.TempFile("", "apollo_file_edit_")
	fname := f.Name()
	f.Close()
	os.Remove(fname)
