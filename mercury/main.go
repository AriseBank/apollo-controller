...
func main() {
	...
	if err := run(); err != nil {
		...
		apolloErr := getLocalErr(err)
		switch apolloErr {
		case syscall.ENOENT:
			msg = i18n.G("APOLLO socket not found; is APOLLO installed and running?")
		case syscall.ECONNREFUSED:
			msg = i18n.G("Connection refused; is APOLLO running?")
		case syscall.EACCES:
			msg = i18n.G("Permission denied, are you in the apollo group?")
		}
...
func run() error {
	...
	if os.Getenv("APOLLO_CONF") != "" {
		configDir = os.Getenv("APOLLO_CONF")
	} else if os.Getenv("HOME") != "" {
		configDir = path.Join(os.Getenv("HOME"), ".config", "mercury")
	} else {
		...
		configDir = path.Join(user.HomeDir, ".config", "mercury")
	}
	configPath = os.ExpandEnv(path.Join(configDir, "config.yml"))

	if len(os.Args) >= 3 && os.Args[1] == "config" && os.Args[2] == "profile" {
		fmt.Fprintf(os.Stderr, i18n.G("`mercury config profile` is deprecated, please use `mercury profile`")+"\n")
	...
	// if APOLLO has been properly configured.  Don't display the message if the var path
	// does not exist (APOLLO not installed), as the user may be targeting a remote daemon.
	if os.Args[0] != "help" && os.Args[0] != "version" && shared.PathExists(shared.VarPath("")) && !shared.PathExists(conf.ConfigDir) {
		...
		fmt.Fprintf(os.Stderr, i18n.G("If this is your first time using APOLLO, you should also run: apollo init")+"\n")
		fmt.Fprintf(os.Stderr, i18n.G("To start your first container, try: mercury launch ubuntu:16.04")+"\n\n")
	}
...
// defaultAliases contains MERCURY's built-in command line aliases.  The built-in
...
func expandAlias(conf *config.Config, origArgs []string) ([]string, bool) {
	 ...
	 * aliases, e.g. `mercury list` to `mercury list -c n`
	 */
