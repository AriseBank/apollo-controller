...
func (p *ProgressRenderer) UpdateProgress(progress apollo.ProgressData) {
...
func containerDeviceAdd(client apollo.ContainerServer, name string, devName string, dev map[string]string) error {
...
func profileDeviceAdd(client apollo.ContainerServer, name string, devName string, dev map[string]string) error {
...
func cancelableWait(op *apollo.RemoteOperation, progress *ProgressRenderer) error {
	// Signal handling
...
func ensureImageAliases(client apollo.ContainerServer, aliases []api.ImageAlias, fingerprint string) error {
	if len(aliases) == 0 {
		return nil
	}
